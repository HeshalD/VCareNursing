...
exports.convertToBooking = async (req, res) => {
    const { request_id, quote_id, slip_url } = req.body;
    ...
    try {
        ...
        await client.query(
            `INSERT INTO bookings (client_id, patient_id, service_type, start_date) VALUES ($1, $2, $3, $4)`,
            [clientProfileId, newPatient.rows[0].patient_id, reqData.service_type, reqData.start_date]
        );
        ...
        // 6. Send Welcome WhatsApp (Step 7 & 8 in Client's Doc)
        const welcomeMsg = `*Booking Confirmed!* \nWelcome to VCare. Your account has been created.\n` +
            `Login: vcarenursing.com\n` +
            (reqData.tempPassword ? `Temp Password: ${reqData.tempPassword}` : `Use your existing password.`);
        
        await sendWhatsAppMessage(reqData.payer_mobile, welcomeMsg);

        res.status(200).json({ status: 'success', message: "Account created and booking activated." });
