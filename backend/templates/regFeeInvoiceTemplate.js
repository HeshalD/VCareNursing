// Registration Fee Invoice PDF template.
//
// Expected `data` shape:
//   {
//     invoice_code,       // e.g. "INV-CL-00023"
...
//     account_number,
//     branch_name,
//     upload_url,         // full URL to the receipt upload portal
//   }
module.exports = (data) => {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .bill-to .meta { font-size: 11px; color: #6b7280; line-height: 1.6; }
        ...
        .bank-block { background: #f8fafc; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px 20px; margin-bottom: 24px; }
        .bank-block .bank-title { font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: #9ca3af; margin-bottom: 12px; }
        .bank-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px 28px; }
        .bank-row .bk-label { font-size: 10px; color: #9ca3af; margin-bottom: 2px; }
        .bank-row .bk-value { font-size: 12px; font-weight: 600; color: #111827; }

        .upload-block { background: #ecfdf5; border: 1px solid #6ee7b7; border-radius: 8px; padding: 14px 18px; margin-bottom: 24px; }
        .upload-block p { font-size: 11.5px; color: #065f46; line-height: 1.6; }
        .upload-block .upload-url { font-size: 10px; color: #047857; word-break: break-all; margin-top: 6px; font-family: monospace; }
...
<body>
    ...
    <div class="title-bar">
        <h1>Invoice</h1>
        <div class="inv-no">${data.invoice_code || ''}</div>
    </div>
        ...
        <div>
            ...
            <div class="inv-detail" style="margin-top: 16px;">
                <table>
                    ...
                    <tr><td>Due Date</td><td>${data.invoice_date || '—'}</td></tr>
                </table>
            </div>
        </div>
        <div class="amount-banner">
            <div class="ab-label">Amount Due</div>
            <div class="ab-amount">LKR ${fmt(data.amount)}</div>
            <div class="ab-due">Payment due upon receipt</div>
        </div>
    </div>
    ...
    <table class="items-table">
        ...
        <tbody>
            <tr>
                <td>
                    <div class="item-label">Registration Fee</div>
                    <div class="item-desc">One-time VCare Nursing client registration fee</div>
                </td>
    ...
    <div class="bank-block">
        <div class="bank-title">Payment Instructions — Bank Transfer</div>
        <div class="bank-grid">
            <div class="bank-row">
                <div class="bk-label">Bank</div>
                <div class="bk-value">${data.bank_name || '—'}</div>
            </div>
            <div class="bank-row">
                <div class="bk-label">Account Name</div>
                <div class="bk-value">${data.account_holder_name || '—'}</div>
            </div>
            <div class="bank-row">
                <div class="bk-label">Account Number</div>
                <div class="bk-value">${data.account_number || '—'}</div>
            </div>
            <div class="bank-row">
                <div class="bk-label">Branch</div>
                <div class="bk-value">${data.branch_name || '—'}</div>
            </div>
        </div>
    </div>
    ...
    <div class="upload-block">
        <p>
            Once you have completed the payment, please upload your bank slip or payment confirmation using the link below.
            Our team will verify and update your registration status shortly.
        </p>
        <div class="upload-url">${data.upload_url || ''}</div>
    </div>

    <div class="footer">
        Generated on ${data.invoice_date || ''} &nbsp;·&nbsp; <strong>VCare Nursing</strong> &nbsp;·&nbsp; No: 293/17B, Hospital Rd, Sri Jayewardenepura<br>
