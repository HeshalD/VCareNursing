      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex justify-center mb-12">
          <div className="flex items-center gap-4">
            {[
              { id: 1, title: "Select Caregiver", icon: Heart },
              { id: 2, title: "Payer Details", icon: User },
              { id: 3, title: "Patient Info", icon: UserCheck },
              ...
              <React.Fragment key={step.id}>
                ...
                {index < 3 && (
                  <div className={`h-0.5 w-16 transition-all ${
                    currentStep > step.id ? 'bg-amber-600' : 'bg-slate-300'
                  }`} />
                )}
              </React.Fragment>
            ))}
              ...
              <AnimatePresence mode="wait">
                ...
                {currentStep === 4 && (
                  <motion.div
                    ...
                    className="space-y-6"
                  >
                    <h2 className="text-2xl font-bold text-slate-800 mb-6">Confirm Booking Details</h2>

                    {/* Selected Caregiver */}
                    {selectedStaff && (
                      <div className="bg-amber-50 rounded-xl p-6 border border-amber-100">
                        <h3 className="font-bold text-amber-900 mb-4">Selected Caregiver</h3>
                        <div className="flex items-center gap-4">
                          <img
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-600">{selectedStaff.role} • {selectedStaff.experience}</p>
                            <p className="text-sm font-semibold text-amber-600">{selectedStaff.price}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="bg-slate-50 rounded-xl p-6 border border-slate-100">
                      <h3 className="font-bold text-slate-900 mb-4">Payer Information</h3>
                      <div className="space-y-2">
                        <p className="text-slate-700"><strong>Name:</strong> {formData.payer_name}</p>
                        <p className="text-slate-700"><strong>Mobile:</strong> {formData.payer_mobile}</p>
                        {formData.payer_email && <p className="text-slate-700"><strong>Email:</strong> {formData.payer_email}</p>}
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 rounded-xl p-6 border border-slate-100">
                      ...
                      <div className="space-y-2">
                        <p className="text-slate-700"><strong>Name:</strong> {formData.patient_name}</p>
                        <p className="text-slate-700"><strong>Age:</strong> {formData.patient_age} years</p>
                        <p className="text-slate-700"><strong>Relationship:</strong> {formData.relationship}</p>
                        <p className="text-slate-700"><strong>Address:</strong> {formData.home_address}</p>
                        <p className="text-slate-700"><strong>Start Date:</strong> {new Date(formData.start_date).toLocaleDateString()}</p>
                        {formData.patient_condition && (
                          <p className="text-slate-700"><strong>Condition:</strong> {formData.patient_condition}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                      ...
                      <p className="text-sm text-emerald-900">
                        By confirming this booking, you agree to our terms of service. A VCare representative will contact you within 24 hours to finalize the arrangements.
                      </p>
                    </div>
                  </motion.div>
                )}
              ...
              <div className="pt-8 mt-8 border-t border-slate-100 flex justify-between">
                {currentStep > 1 ? (
                  <button
                    type="button"
                    onClick={prevStep}
                    className="flex items-center gap-2 px-6 py-3 text-slate-600 font-medium hover:text-amber-600 transition-colors"
                  >
                ...
                {currentStep < 4 ? (
                  <button
                    type="button"
                    onClick={nextStep}
                    disabled={currentStep === 1 && !selectedStaff}
                    className="flex items-center gap-2 px-8 py-3 bg-amber-600 text-white rounded-xl font-bold shadow-lg shadow-amber-500/30 hover:bg-amber-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
