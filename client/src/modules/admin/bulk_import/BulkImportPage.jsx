...
const BulkImportPage = () => {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      setError('');
      setCommitResults(null);
      const res = await apiClient.previewBulkImport(selectedFile);
  ...
  const handleCommit = async () => {
    ...
    try {
      setCommitting(true);
      setError('');
      const res = await apiClient.commitBulkImport(selectedFile);
  ...
  return (
    <AdminLayout title="Bulk Import" subtitle="Migrate staff, clients, care profiles, and bookings from a spreadsheet.">
      <div className="max-w-4xl">
        ...
        {previewResults && (
          <div className="bg-white border border-gray-200 rounded-lg p-5 mb-6">
            <h3 className="font-semibold text-gray-800 mb-3">Preview results</h3>
            {Object.keys(SHEET_LABELS).map((key) => (
              <ResultTable key={key} label={SHEET_LABELS[key]} rows={previewResults.results[key]} />
            ))}
            <button
        ...
        {commitResults && (
          <div className="bg-white border border-gray-200 rounded-lg p-5">
            <h3 className="font-semibold text-gray-800 mb-1">Import complete</h3>
            <p className="text-sm text-gray-600 mb-3">
              Batch ID: <span className="font-mono">{commitResults.import_batch_id}</span>
            </p>
            {Object.keys(SHEET_LABELS).map((key) => (
              <ResultTable key={key} label={SHEET_LABELS[key]} rows={commitResults.results[key]} />
            ))}
          </div>
        )}
      </div>
